...
class Program
{
    ...
    static async Task Main(string[] args)
    {
        ...
        // Start Web API for Figma Integration
        var builder = WebApplication.CreateBuilder(args);
        var app = builder.Build();

        app.MapGet("/", async (HttpContext context) =>
        {
            await context.Response.WriteAsync("✅ Server is running! Use POST /play_audio to trigger audio.");
        });

         // Route: Trigger audiobook + haptic effects from Figma
        app.MapGet("/play_audio", async (HttpContext context) =>
        {
            await StartAudioAndHaptics();
            await context.Response.WriteAsync("🎧 Playing audiobook with haptics!");
        });
    ...
    static async Task StartAudioAndHaptics()
    {
        Console.WriteLine("Triggered: Playing audiobook & starting haptics...");

        // Play the audiobook
        string path = Path.Combine(Directory.GetCurrentDirectory(), "F451 Script Pt 2.wav");
        ...
        await Task.Delay(10000); // Adjust delay for your sequence
        await resetDots();
    }
